...
{
public:
    ObjCol(const std::shared_ptr<CollisionDetector>& colDetector);
    ~ObjCol();
...
protected:
    ...
    // develop only
    void RenderIntersection(const std::shared_ptr<Renderer>& renderer, bool isPermitCamera, const std::weak_ptr<Package>& package) const;
